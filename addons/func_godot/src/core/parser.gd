...
func parse_map_data(map_file: String, map_settings: FuncGodotMapSettings) -> _ParseData:
	...
	if map_file.begins_with("uid://"):
		...
		if not ResourceUID.has_id(uid):
			_T.say("Error: failed to retrieve path for UID (%s)" % map_file, Color.RED)
			return parse_data
	...
	if not file:
		...
		if file:
			...
			_T.say("Error: Failed to open map file (" + map_file + ")", Color.RED)
			return parse_data
	...
	if map_file.ends_with(".import"):
		while not file.eof_reached():
			...
			if line.begins_with("path"):
				...
				if data.is_empty():
					_T.say("Error: Failed to open map file (" + line + ")", Color.RED)
					return parse_data
				map_data = data.split("\n")
				break
	else:
