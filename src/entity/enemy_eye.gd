...
func _ready() -> void:
	...
	#chase_component.set_target(_G.player.target.get_pos_multiplied(0.3 + enemy.random_factor))
...
func _physics_process(delta : float) -> void:
	...
	if chase_component.attacking and _G.player.can_control:
		enemy.shoot_to_player(shoot_component, 0.5, 0.0)
		
	if is_latched:
		pass
	else:
		chase_component.update_flying(_G.player.target.global_position, movement_component)
