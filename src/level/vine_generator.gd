...
func _func_godot_build_complete() -> void:
	...
	for i in range(num_vines):
		var rand_pos : Vector3 = Vector3(randf_range(-1.0, 1.0), 0.0, randf_range(-1.0, 1.0)).normalized() \
		* randf_range(-func_godot_properties["radius"], func_godot_properties["radius"])
